...
class ChemicalEncoder(nn.Module):
    ...
    def forward(self, bmg, coords_3d=None):
        ...
        if self.use_3d and coords_3d is not None:
            ...
            # Expand atom features with 3D info
            # NOTE: This requires modifying bmg.V (atom features)
            # We concatenate 3D features to existing atom features
            bmg_modified = bmg  # In practice, need to create new BMG with expanded V
            # bmg_modified.V = torch.cat([bmg.V, coord_features], dim=1)
            # For now, we'll handle this in the dataset/featurizer
